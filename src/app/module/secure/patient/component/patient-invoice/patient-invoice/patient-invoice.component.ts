...
export class PatientInvoiceComponent implements OnInit {
  ...
  find() {
    ...
    if (this.InvoiceStatus !== undefined && this.InvoiceStatus.status === 'Paid') {
      this.searchParamsData.InvoiceStatuses = '4,5,6,7,8,9';
    } else {
      ...
      (findInvoiceResponse: any) => {
        ...
        if (findInvoiceResponse.hasOwnProperty('data') && findInvoiceResponse['data'].length === 0) {
          ...
          this.invoiceList.forEach(element => {
            ...
            // if (element.invoiceStatus == 5 && element.invoiceType == 3) {
            //   element.displayInvoiceStatus = 'Subscription Plan Created';
            // }
  ...
  getAggregateAmountObject(invoiceData) {
    ...
    //   columns: [
    //     {
    //       text: DownloadToPDF_Invoice['subTotal'] + ': ' + invoiceData.subTotal,
    //       alignment: 'right'
    //     }
    //   ]
    // });

    // if (invoiceData.discountType === 1) {
    //   aggregates.push({
    //     columns: [
    //       {
    //         text: DownloadToPDF_Invoice['discount'] + ': ' + invoiceData.calculatedDiscount,
    //         alignment: 'right'
    //       }
    //     ]
    //   });
    // } else if (invoiceData.discountType === 2) {
    //   aggregates.push({
    //     columns: [
    //       {
    //         text: DownloadToPDF_Invoice['discount'] + '(' + (invoiceData.discountPercent) + '%)' + ': ' + invoiceData.calculatedDiscount,
    //         alignment: 'right'
    //       }
    //     ]
    //   });
    ...
    //   columns: [
    //     {
    //       text: DownloadToPDF_Invoice['discount'] + '(' + (invoiceData.taxPercent) + '%)' + ': ' + invoiceData.taxAmount,
    //       alignment: 'right'
    //     }
    //   ]
    // });
