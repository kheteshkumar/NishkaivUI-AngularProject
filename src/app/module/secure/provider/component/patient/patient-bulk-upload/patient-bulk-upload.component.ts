...
import { IUploadLogItem, IUploadLogItemContext } from 'src/app/common/interface/uploadLog';
...
@Component({
  selector: 'app-patient-bulk-upload',
  templateUrl: './patient-bulk-upload.component.html',
  styleUrls: ['./patient-bulk-upload.component.scss']
})
export class PatientBulkUploadComponent implements OnInit {

  @ViewChild('dropzonereset') dropzone: ElementRef;
  @ViewChild(UploadFileComponent) UploadFileComponent: UploadFileComponent;
  @ViewChild('modalUploadPatient') modalUploadPatient: ModalTemplate<
  ...
  sortPatients(inputData) {
    ...
    this.uploadLogs$ = this.fetchUploadLogs(1);
  }
  ...
  fetchUploadLogs(pageNumber: number = 1) {
    ...
    const uploadLogs$ = this.patientUploadsService.getAllUploadLogs(this.searchParamsData).pipe(
      catchError((error) => {
  ...
  getFileStatusText(status: UploadFileStatusEnum) {
    return this.patientUploadsService.getFileStatusText(status);
  }

  uploadFile() {
    this.UploadFileComponent.submit();
  }
  ...
  outputDataFromOperation(OutputData) {
    ...
    if (OutputData.error) {
      ...
      this.toastData = this.toasterService.success(MessageSetting.patient.uploadSuccess);
      setTimeout(() => {
        this.toastData = this.toasterService.closeToaster(MessageSetting.patient.uploadSuccess);
      }, 5000);
    }
  ...
  // Add Patient Modal
  public uploadPatientModal() {
    if (this.ifModalOpened) { // To avoid opening of multiple modal
      return;
    }
    const config = new TemplateModalConfig<IUploadLogItemContext, IUploadLogItem, IUploadLogItem>(this.modalUploadPatient);
  ...
  downloadTemplate() {
    ...
    let a = Object.keys(PatientUploadRecordEnum)
      .map(label => (PatientUploadRecordEnum[label]));
    ...
    Utilities.downloadCsv(csv, 'patient-template.csv');
