...
import { DatepickerMode, TransitionController, ModalTemplate, SuiModalService, TemplateModalConfig, Transition, TransitionDirection } from 'ng2-semantic-ui';
...
import { FrequencyEnum, InvoiceFrequencyEnum } from 'src/app/enum/billing-execution.enum';
import { FeaturesEnum } from 'src/app/enum/features.enum';

import { InvoiceStatusEnum, InvoiceStatusMapEnum, InvoiceTypeEnum, InvoiceTypeMapEnum, InvoiceStatusReportEnum } from 'src/app/enum/invoice.enum';
import { RecurringPaymentTypeEnum } from 'src/app/enum/recurring-payment-type.enum';
...
import { ConfirmModal, IContext } from 'src/app/module/secure/patient/component/patient-transaction/patient-transaction/patient-transaction.component';
...
@Component({
  selector: 'app-find-payment-plan',
  templateUrl: './find-payment-plan.component.html',
  styleUrls: ['./find-payment-plan.component.scss']
})
export class FindPaymentPlanComponent implements OnInit {
  //chart
  lineChart;
  lineChartLabels = [];
  dataSet = [];
  ...
  invoiceStatusSelected = [
    InvoiceStatusEnum.PaymentPlanCreated,
    InvoiceStatusEnum.OneTimeScheduledCreated,
    InvoiceStatusEnum.SubscriptionPlanCreated,
    InvoiceStatusEnum.InProgress,
  ...
  ngOnInit() {
    ...
    if (this.invoiceService.getSelectedStatuses() === true) {
      this.findInvoiceForm.patchValue({
        Status: [InvoiceStatusEnum.PaymentPlanCreated, InvoiceStatusEnum.InProgress]
      })
      this.displayInvoiceStatusFilter = [];
      this.displayInvoiceStatusFilter.push(InvoiceStatusMapEnum['PaymentPlanCreated']);
      this.displayInvoiceStatusFilter.push(InvoiceStatusMapEnum['InProgress']);
      this.invoiceService.setSelectedStatuses(); // reset the filter value
    }
    ...
    this.find(true);
    if (this.permissions.invoiceReports) {
      this.fetchInvoiceStatusReport();
    }
  }
  fetchInvoiceStatusReport() {
    this.invoiceService.getInvoiceStatusReport({ InvoiceStatuses: this.invoiceStatusSelected.toString() }).subscribe((response: any) => {
      //console.log("reponse ::" + JSON.stringify(response))
      this.lineChartLabels = [];
      this.dataSet = []
      const countStatusReportSet = { label: [], data: [], backgroundColor: [], hoverOffset: 4, totalFinalAmount: [] };
      response.forEach((report, i) => {
        this.lineChartLabels.push(report.InvoiceStatus)
        countStatusReportSet.data.push(report.TotalCount)
        countStatusReportSet.label.push(report.InvoiceStatus)
        countStatusReportSet.totalFinalAmount.push(report.TotalFinalAmount)
        countStatusReportSet.backgroundColor.push(this.color[i])
      })
      this.dataSet.push(countStatusReportSet)
  ...
  drawLineChart() {
    ...
    const that = this;
    this.lineChart = new Chart('canvasStatus', {
      type: 'doughnut',
      data: {
      ...
      options: {
        onClick: function (e, i) {
          ...
          if (i.length > 0) {
            let item: any = i[0];
            let invoiceStatusSelected = InvoiceStatusReportEnum[item._chart.config.data.datasets[item._datasetIndex].label[item._index]];
            that.findCalled(invoiceStatusSelected);
          }
        },
        responsive: true,
        legend: {
          ...
          onClick: function (event, legendItem) {
            ...
            that.updateGraph(legendItem.index, legendItem.text);
          }
        },
        tooltips: {
          callbacks: {
            footer: function (t, d) {
              let tempT: any = t[0]
              let tempD: any = d
              return 'Amount : $' + formatNumber(Number(tempD.datasets[tempT.datasetIndex].totalFinalAmount[tempT.index]), "en-US", '1.2-2'); //return a string that you wish to append
            }
          }
        }
      }
  ...
  updateGraph(index, label) {
    let ci = this.lineChart;
    var meta = ci.getDatasetMeta(0).data[index];
    // show hide dataset
    meta.hidden = !meta.hidden;
    const status = parseInt(InvoiceStatusReportEnum[label]);
    if (meta.hidden) {
      let idx = this.invoiceStatusSelected.indexOf(status);
      if (idx > -1) {
        this.invoiceStatusSelected.splice(idx, 1);
      }
    } else {
      this.invoiceStatusSelected.push(status)
    }
    //console.log("this.invoiceStatusSelected "+this.invoiceStatusSelected)
    if (this.invoiceStatusSelected.length == 0) {
      this.invoiceList = [];
    } else {
      this.findCalled(this.invoiceStatusSelected);
    }
    //rerender the chart
    ci.update();
  }
  ...
  OnCheckChange(selectedcheck, value) {
    let open = selectedcheck == 'open' ? value : this.findInvoiceForm.value.eCheckOpen;
    let closed = selectedcheck == 'closed' ? value : this.findInvoiceForm.value.eCheckClosed
    if (open && closed) {
      this.invoiceStatusSelected = [
        InvoiceStatusEnum.PaymentPlanCreated,
        InvoiceStatusEnum.OneTimeScheduledCreated,
        InvoiceStatusEnum.SubscriptionPlanCreated,
        InvoiceStatusEnum.InProgress,
        ...
        InvoiceStatusEnum.Unsubscribed,
  ...
  patientLookUp() {
    const reqObj = { 'searchTerm': '', isActive: true, isRegistered: true, SortField: 'CreatedOn', Asc: false };
  ...
  prepareReqObjForGetAll() {
    ...
    reqObj.InvoiceStatuses = this.invoiceStatusSelected.toString();
    reqObj.InvoiceType = "1,2,3"; //one time shceduled,subscription plan and payment plan

    // if (this.invoiceService.getSelectedStatuses() === true) {
    //   reqObj.InvoiceStatuses = InvoiceStatusEnum.PaymentPlanCreated + ',' + InvoiceStatusEnum.InProgress;
    //   this.invoiceService.setSelectedStatuses(); // reset the filter value
    // } else 
    if (this.findInvoiceForm.value.Status != null && this.findInvoiceForm.value.Status !== ''
      && this.findInvoiceForm.value.Status !== undefined && this.findInvoiceForm.value.Status.length > 0) {
      reqObj.InvoiceStatuses = this.findInvoiceForm.value.Status;
    }
  ...
  find(initiatePager: boolean = false) {
    ...
    this.searchParamsData = this.prepareReqObjForGetAll();
  ...
  getInvoiceById(invoice, confirmPaymentModal?: string) {
    ...
    this.getAllTransactionByInvoiceNumber(invoice.invoiceNumber);
  ...
  getAllTransactionByInvoiceNumber(invoiceNumber) {
